...
const CalificacionesTerman = require("../../model/terman/calificacionesTerman.model");
const ResultadosSeriesTerman = require("../../model/terman/resultadosSeriesTerman.model");
...
async function calificarSerieTerman(idSerie, idUsuario, idGrupo, respuestas) {
  ...
  const calificacion = await calificacionesModel.fetchCalificacionById(
    idUsuario,
    idGrupo
  );
  ...
  const opcionesCorrectas = await ModeloTerman.fetchOpcionesCorrectasById(
    idSerie
  );
  ...
  if (!handler) {
    throw new Error(
      `No hay función de calificación definida para la serie ${idSerie}`
    );
  }
...
function calificarSerie1(respuestas, opcionesCorrectas) {
  ...
  const correctas = new Set(
    opcionesCorrectas.map((op) => `${op.idPreguntaTerman}-${op.opcionTerman}`)
  );
